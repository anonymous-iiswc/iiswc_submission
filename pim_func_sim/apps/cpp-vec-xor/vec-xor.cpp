...
void vectorXOR(uint64_t vectorLength, std::vector<int> &src1, std::vector<int> &src2, std::vector<int> &dst)
{
  ...
  status = pimXor(srcObj1, srcObj2, srcObj1);
  if (status != PIM_OK)
  {
    std::cout << "Abort" << std::endl;
    return;
...
int main(int argc, char* argv[])
{
  ...
  vectorXOR(params.vectorLength, src1, src2, dst);
  if (params.shouldVerify) {
    ...
    for (unsigned i = 0; i < params.vectorLength; ++i)
    {
      ...
      if (dst[i] != sum)
      {
        std::cout << "Wrong answer for xor: " << src1[i] << " ^ " << src2[i] << " = " << dst[i] << " (expected " << sum << ")" << std::endl;
      }
    }
  }
