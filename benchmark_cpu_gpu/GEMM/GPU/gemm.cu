...
int main(int argc, char *argv[])
{
    ...
    u64 row = p.row, col_A = p.column_A, col_B = p.column_B;

    initVector(row * col_A, A);
    initVector(col_A * col_B, B);

    float *x, *y, *z;

    cudaError_t errorCode;

    errorCode = cudaMalloc(&x, row * col_A * sizeof(data_t));
    if (errorCode != cudaSuccess)
    {
        cerr << "Cuda Error: " << cudaGetErrorString(errorCode) << "\n";
        exit(1);
    }
    errorCode = cudaMalloc(&y, col_A * col_B * sizeof(data_t));
    if (errorCode != cudaSuccess)
    {
        cerr << "Cuda Error: " << cudaGetErrorString(errorCode) << "\n";
        exit(1);
    }
    errorCode = cudaMalloc(&z, row * col_B * sizeof(data_t));
    if (errorCode != cudaSuccess)
    {
        cerr << "Cuda Error: " << cudaGetErrorString(errorCode) << "\n";
        exit(1);
