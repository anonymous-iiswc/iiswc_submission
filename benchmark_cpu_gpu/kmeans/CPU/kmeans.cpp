...
inline data_t calculateDistance(const vector<data_t> &pointA, const vector<data_t> &pointB, int dim) {
    data_t sum = 0;
    for (int i = 0; i < dim; i++)
    {
...
{
    clusters.resize(k, vector<data_t>(dimension));
...
int main(int argc, char **argv)
{
    ...
    initMatrix(numPoints, dim, dataPoints);
