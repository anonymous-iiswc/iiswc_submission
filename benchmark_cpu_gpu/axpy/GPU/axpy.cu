...
int main(int argc, char *argv[])
{
    ...
    errorCode = cudaMalloc((void **)&x, vector_size * sizeof(data_t));
    if (errorCode != cudaSuccess)
    {
        cerr << "Cuda Error: " << cudaGetErrorString(errorCode) << "\n";
        exit(1);
    }
    errorCode = cudaMalloc((void **)&y, vector_size * sizeof(data_t));
    if (errorCode != cudaSuccess)
    {
        cerr << "Cuda Error: " << cudaGetErrorString(errorCode) << "\n";
        exit(1);
    ...
    vector<data_t> C(vector_size);
    errorCode = cudaMemcpy(C.data(), y, vector_size * sizeof(data_t), cudaMemcpyDeviceToHost);
    if (errorCode != cudaSuccess)
    {
